...
- HI Good morning My name is Vinay and I am here to present my Demo on Private development Preview over Tailscale.

---

## 2. Agenda

- We will Walk thru the arc of developer pain → my personal story → Why I decided to use Tailscale? → the use case → its architecture → live demos → access controls → proofs of it working → VPN contrast → My reflection, things that I faced → value for newcomers → and at the end we will have Q&A session
...
- Second scenario: you prototyped something on your laptop. You want a friend or teammate to click through it. You do not want to publish to the public web.
...
- I have tried looking for various solutions to solve these  problems before
...
- I have used private VPN / virtual LAN tools to play with friends as if we were on the same network. In practice it felt slow, fiddly to set up, and annoying to keep working with because the connections would just keep dropping.
...
- It is Gateway-centric, too slow for a simple private preview. Developers don’t have time for all this.
...
- NO need to raise tickets for “whitelist someones laptop’s IP and hope it doesn’t change.”
- MagicDNS is easier to remember customizable and not a bunch of numbers for a development site.
- Easy integration with existing CI/CD.
- 
- Bottom line is Cosco is built for classic corporate remote access. 
- Tailscale is built for connecting people and services by identity, which matches private staging, contractor preview  patterns.
- It works across laptops, phones, cloud VMs, and CI without redesigning hub-and-spoke.
- Fewer moving parts for the developer means less friction — and less friction means easier adoption.
...
- Understand what CICD is.  If not dont know I can explain it at the end with the help of analogies.
...
- Number 1 **Browse:** Laptop or phone with Tailscale can access my MagicDNS and site. 
...
- `tag:ci` are ephemeral nodes with least privilege for deploy only. It has permission to access only dev-server.
- How can I do this kind of setup? 
- Using ACL's (show ACL) Bascially ACL's allow for such kind of access controls which are modifiable by the developer
...
I have used 2 different types of keys for different purposes

- **1 · EC2 auth key for** Bootstraping SSH from my allowlisted IP → `tailscale up --auth-key` → This produces a stable MagicDNS hostname which I can consistantly use. As u swa before how the EC2 machine values changed everytime I restarted the service. Long-lived server stays on the Tailnet.
- **2 · CI OAuth for** GitHub runners because runners can show up from **any IP**. You still want them to reach EC2, but the security group is tight (SSH demo: public SSH only from your IP). you Don’t open AWS for every Actions IP.
- Instead the runner joins the Tailnet automatically via OAuth, mints ephemeral `tag:ci`, SSHs over the mesh, deploys, then leaves
- OAuth is scoped to mint `tag:ci` only; ACL only lets `tag:ci` talk to `tag:dev-server` on SSH.
...
- **Tailscale mesh:** on the other hand, allows Devices to join by identity. Laptop, phone, teammate, CI, MagicDNS, and EC2 share one Tailnet. These are all Peer links. Tags and ACLs decide who reaches what and where.
...
- Bootstrap still needs a public SSH path (or SSM) before Tailscale is up.
...
- After **revoking auth keys or machines** and then added back the same nodes, they sometimes appeared online on the Tailnet but **SSH still failed** (stale identity / host key / Tailscale SSH policy leftovers).
...
- I’d dig into a **Docker** packaging of this pattern and how it would run inside a **Kubernetes** cluster 
...
- Still want deeper ACL / grants demos for multi-team least privilege type of patterns
- Explore **Tailscale API** features to automatically **add or remove users/machines** from the mesh based on API calls
